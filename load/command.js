...
const loadAllCommands = async () => {
    ...
    const commandDirs = readdirSync(commandRoot).filter(file => lstatSync(path.join(commandRoot, file)).isDirectory());
...
const loadCategory = async (category,commands) =>{
    try{
        const commandFiles = readdirSync(path.join(commandRoot,category)).map(cmd => `../Commands/${category}/${cmd}`);
        for(const cmd of commandFiles){
            const command = await loadCommand(cmd);
            if(!command.name){console.log(`[${cmd.split('/')[3]}] 명령어 이름 누락! 무시 됨.`); continue;}
            if(command){
