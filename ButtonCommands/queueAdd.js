...
const intertSong = async ({player,songID,requestor,webhook}) =>{
    ...
    if(player.isPlay){ 
        webhook.send(music.embed.setAuthor('📥 노래 추가되었습니다.').setColor('BLUE'))
    }else {
        player.next();
    }
    return;
...
module.exports = {
    ...
    execute: async ({cmd,msg,excutor,followupWebhook}) => {
        ...
        if(excutor.voice.channel.id !== player.voice.id) return reply('봇이 입장한 보이스 채널에 접속하세요.');
         ...
         intertSong({player,songID:cmd[1],requestor:excutor,webhook:followupWebhook});
         return {
            type:7,
            data:{
                content: `💽  <@${excutor.id}> 님이 음악을 선택하셨습니다.`,components: []
            }
        };
    }
}
